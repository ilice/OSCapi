...
class TestOfCatastro extends UnitTestCase {
	...
	function testCatastroGetRCForLatitudeAndLongitude() {
		...
		$web_folder = "Web/";
		$url = $server_name . "/" . $web_folder . "php/catastro.php?end_point=" . $end_point . "&SRS=EPSG:4326&Coordenada_X=" . $longitud . "&Coordenada_Y=" . $latitud;
		...
		if ($response_xml->coordenadas->count () > 0) {
	...
	function testCatastroGetProvinciaForRC() {
		...
		$this->assertNotEqual ( $response_xml, false, 'Debe ser xml y parsearse correctamente. Parseando: ' . $response . " para la url: " . $url );
		$this->assertEqual ( $response_xml->control [0]->cuerr, 0, 'No debe haber errores  para la url: ' . $url . ' y hay ' . $response_xml->control [0]->cuerr );
		$this->assertEqual ( $response_xml->provinciero->count (), 1, 'Debe haber un resultado para la url: ' . $url . ' y hay ' . $response_xml->coordenadas->count () );
		if ($response_xml->provinciero->count () > 0) {
	...
	function testCatastroGetMunicipioForRC() {
		...
		$this->assertNotEqual ( $response_xml, false, 'Debe ser xml y parsearse correctamente. Parseando: ' . $response . " para la url: " . $url );
		$this->assertEqual ( $response_xml->control [0]->cuerr, 0, 'No debe haber errores  para la url: ' . $url . ' y hay ' . $response_xml->control [0]->cuerr );
		$this->assertEqual ( $response_xml->municipiero->count (), 1, 'Debe haber un resultado para la url: ' . $url . ' y hay ' . $response_xml->coordenadas->count () );
		if ($response_xml->municipiero->count () > 0) {
	...
	function testCatastroGetDatosCatastroForRC() {
		...
		$this->assertNotEqual ( $response_xml, false, 'Debe ser xml y parsearse correctamente. Parseando: ' . $response . " para la url: " . $url );
		$this->assertEqual ( $response_xml->control [0]->cuerr, 0, 'No debe haber errores  para la url: ' . $url . ' y hay ' . $response_xml->control [0]->cuerr );
		$this->assertEqual ( $response_xml->bico->count (), 1, 'Debe haber un resultado para la url: ' . $url . ' y hay ' . $response_xml->coordenadas->count () );
		if ($response_xml->bico->count () > 0) {
