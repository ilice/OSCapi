...
class TestOfInforiego extends UnitTestCase {
	...
	function testInforiegoGetEstaciones() {
		$server_name = ! empty ( $_SERVER ['SERVER_NAME'] ) ? $_SERVER ['SERVER_NAME'] : "http://localhost:8080";
		$web_folder = "Web/";
		$url = $server_name . "/" . $web_folder . "php/inforiego_rest.php?accion=obtenEstacion&numeroEstaciones=20";
		
		$response = $this->getResponse ( $url );
		
		$response_json = json_decode ( utf8_encode ( $response ), true );
		$error = json_last_error_msg ();
		$this->assertEqual ( $error, 'No error', 'Se han producido los siguiente errores : ' . $error . ' al parsear ' . $response );
		$this->assertTrue ( count($response_json) > 10, 'El número de estaciones debe ser mayor que 10 mediante la url: ' . $url );
		
	}
	
	function testInforiegoGetEstacion() {
		$server_name = ! empty ( $_SERVER ['SERVER_NAME'] ) ? $_SERVER ['SERVER_NAME'] : "http://localhost:8080";
		$web_folder = "Web/";
		$latitud = 40.439983;
		$longitud = - 5.737026;
		$url = $server_name . "/" . $web_folder . "php/inforiego_rest.php?accion=obtenEstacion&latitud=" . $latitud . "&longitud=" . $longitud ;
		...
		$error = json_last_error_msg ();
		$this->assertEqual ( $error, 'No error', 'Se han producido los siguiente errores : ' . $error . ' al parsear ' . $response );
		$this->assertTrue ( count($response_json) == 1, 'El número de estaciones debe ser 1 mediante la url: ' . $url );
	...
	function testInforiegoGetLluviaYPrecipitacionAcumuladaForLatitudeAndLongitudeAndYear() {
		...
		$server_name = ! empty ( $_SERVER ['SERVER_NAME'] ) ? $_SERVER ['SERVER_NAME'] : "http://localhost:8080";
		$web_folder = "Web/";
		$url = $server_name . "/" . $web_folder . "php/inforiego_rest.php?accion=diasDeLluvia&latitud=" . $latitud . "&longitud=" . $longitud . "&anio=" . $anio;
	...
	function testInforiegoGetMedidasDiariasForLatitudeAndLongitudeAndYear() {
		...
		$server_name = ! empty ( $_SERVER ['SERVER_NAME'] ) ? $_SERVER ['SERVER_NAME'] : "http://localhost:8080";
		$web_folder = "Web/";
		$url = $server_name . "/" . $web_folder . "php/inforiego_rest.php?accion=medidasDiarias&latitud=" . $latitud . "&longitud=" . $longitud . "&anio=" . $anio;
	...
	function testInforiegoGetDatosPrecipitacionAnualesForLatitudeAndLongitudeAndNoYearAndOthers() {
		...
		$server_name = ! empty ( $_SERVER ['SERVER_NAME'] ) ? $_SERVER ['SERVER_NAME'] : "http://localhost:8080";
		$web_folder = "Web/";
		$url = $server_name . "/" . $web_folder . "php/inforiego_rest.php?accion=datosMedidaPorAnio&latitud=" . $latitud . "&longitud=" . $longitud . "&medida=" . $medida . "&numeroDeAnios=" . $numeroDeAnios . "&intervalo=" . $intervalo . "&formato=" . $formato;
