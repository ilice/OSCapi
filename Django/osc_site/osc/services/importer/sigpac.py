...
def download_shapefile(zip_code,
    ...
    try:
        ...
        zipped_shapefile_path = os.path.join(util.get_tmp_dir(), zipped_shapefile)
...
def get_shapefile(zip_code,
    ...
    working_dir = path(util.get_data_dir(), zip_code=zip_code)
...
def download_shapefiles(zip_codes,
    ...
    for zipCode in zip_codes:
        working_dir = path(util.get_data_dir(), zip_code=zipCode)
...
def write_csv(zip_codes,
    ...
    for zip_code in filter(lambda x: not os.path.exists(csv_file(util.get_data_dir(), x)), util.as_list(zip_codes)):
        ...
        if sf is not None:
            ...
            if df is not None:
                try:
                    ...
                    if not os.path.exists(csv_path(data_dir=util.get_data_dir(), zip_code=zip_code)):
                        os.makedirs(csv_path(data_dir=util.get_data_dir(), zip_code=zip_code))

                    csv = csv_file(data_dir=util.get_data_dir(), zip_code=zip_code)
...
def all_sigpac_zipcodes(starting_with=None,
                        url='ftp.itacyl.es',
                        root_dir='/cartografia/05_SIGPAC/2015_ETRS89/Parcelario_SIGPAC_CyL_Municipios'):
    zip_codes = []
    ftp = ftplib.FTP(url, user='anonymous', passwd='')
...
def read_codigos():
    codigos_path = os.path.join(path(util.get_data_dir()), 'codigos.csv')
...
def save_plots_to_elasticsearch(zip_codes,
                                url='ftp.itacyl.es',
...
def add_altitude_info(provincia, municipio=None):
    ...
    dsl_filter = [dsl.Q("term", provincia=provincia)]
    if municipio is not None:
        dsl_filter.append(dsl.Q("term", municipio=municipio))

    # query elasticsearch for the neccesary registers
    search = dsl.Search(index='plots').query('bool', filter=dsl_filter).fields(['bbox_center.lat', 'bbox_center.lon'])
    search.execute()
